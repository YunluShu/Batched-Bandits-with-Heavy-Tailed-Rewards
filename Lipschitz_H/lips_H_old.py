...
def _ace_sequence(T: int) -> Tuple[List[float], List[int]]:
    edge_length: List[float] = []

    ACE_c: List[float] = [1 / 6 * math.log2(T / math.log2(T))]
    c_cumu = ACE_c[-1]
    eta = 3 / 4
...
def _init_cubes(edge_len0: float) -> List[List[float]]:
    ...
    for i in range(m):
        for j in range(m):
            # x_location, y_location, edge_length, total_reward (sum within batch)
            cubes.append([i * edge_len0, j * edge_len0, edge_len0, 0.0])
    return cubes
